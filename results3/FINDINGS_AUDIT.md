...
| | | > **[NOTE 2026-08-30] Closed, by deleting both lists rather than updating them.** `tests/test_writer_lists.py` derives the writers from the tree — every module under `CODE_ROOTS` matching `write_text(json.dumps` — and asserts three things the hand lists could not: that the set equals the README table **in both directions**, that every writer hangs its `_env`, and that the guarded set is exactly the three modules that spend. `WRITERS` and `FREE` are gone; what they asserted now covers forty writers instead of six and thirteen. `test_provenance.py`'s private copy of `CODE_ROOTS` went with them — it was a second copy of the same tuple and had to be edited twice this week. **What is not closed is the pattern**: the derivation is syntactic, so a module that wrote a record some other way would still slip past, which is the limit the discovery test has carried since 2026-08-07. |
